...
### Fixed
- `hm310t.__version__` was a second, hardcoded copy of the version string,
  independent of `pyproject.toml`. It went stale in the 0.1.1 release: the
  published wheel reports `__version__ == "0.1.0"`. Now derived from installed
  package metadata (`importlib.metadata`), so there's one source of truth.
